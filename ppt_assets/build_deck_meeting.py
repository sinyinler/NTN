...
pic(s,os.path.join(A,'fig3_arch.png'),6.95,1.7,5.9,3.7)
cap(s,6.95,5.45,5.9,"论文 Fig.3：T 的 GIBlock 主路 + 残差做去相关；空间损失对齐高斯分位、频域损失推平功率谱")
...
# ==================== 7 机制硬证据 ====================
s=slide(); motif_title(s,"机制硬证据：T 真把噪声白化了（不是漏噪声）",7)
pic(s,os.path.join(A,'chart_decorr.png'),0.55,1.7,6.5,4.5)
cap(s,0.55,6.3,6.5,"翻译前→后：邻像素相关 lag-1 0.82→0.13；谱平坦度 0.055→0.508（×9，越大越白）")
table(s,7.4,2.0,[3.3,1.9],[
    ["level1 OOD 诊断(10场景)","PSNR"],
    ["D′(I) 不翻译","14.22"],
    ["N2N","19.31"],
    ["D′(T(I)) = NTN","20.34"],
],fs=12,rh=0.52,hl=(3,))
bullets(s,7.4,4.5,5.4,2.6,[
    [("怎么证明翻译机制成立：",13.5,NAVY,True)],
    [("· 不翻译直接喂 D′ → 14.22dB（比 noisy 还差）→ 盲高斯 D′ 单独搞不定真实相关噪声;",12.5,INK,False)],
    [("· 翻译后 20.34 > N2N 19.31；‖T(I)−I‖/‖I‖=0.18 → T 大幅改输入、绝非恒等;",12.5,INK,False)],
    [("· 噪声相关 0.82→0.13、谱平坦 ×9 → 复现论文 Fig.5-7；细节是“翻译得来”不是“漏噪声”。",12.5,INK,False)],
],sz=12.5,gap=7)

# ==================== 8 为什么加数据：手部假血管 ====================
s=slide(); motif_title(s,"为什么要增加数据：内容 OOD 的“假血管”",8)
HAND=os.path.join(R,'images','hand_fake.png')
if os.path.exists(HAND):
    pic(s,HAND,0.55,1.5,8.4,4.7)
else:
    box(s,0.55,1.5,8.4,4.7,fill=SOFT,line=LINE,round_=True)
    text(s,0.55,3.4,8.4,0.8,[[("（人手 noisy / N2N / NTN 假血管对比图：待插入路径）",15,MUTE,True)]],align=PP_ALIGN.CENTER)
cap(s,0.55,6.32,8.4,"人手 BFI：noisy / N2N / NTN + 放大 —— 单被试模型的 NTN 在放大区脑补出原图没有的“假血管”")
bullets(s,9.15,1.8,3.7,4.6,[
    [("现象：",14.5,NAVY,True)],
    [("放大区 NTN ",13,INK,False),("凭空生成细血管网",13,RED,True),("，N2N 没有。",13,INK,False)],
    [("原因：",14.5,NAVY,True)],
    [("去噪器套用训练里的血管先验 → 内容 OOD 脑补；NTN 只管噪声、治不了它。",13,INK,False)],
    [("对策：",14.5,NAVY,True)],
    [("把脑/腿/手等多被试加入训练（下页见结果）。",13,MINT,True)],
],sz=13,gap=8)

# ==================== 9 加数据后：脚 OOD 测试 ====================
s=slide(); motif_title(s,"加数据后：没见过的人脚 OOD 测试也忠实",9)
pic(s,os.path.join(R,'metrics_foot5','compare_gray_color.png'),0.55,1.55,12.2,3.05)
cap(s,0.55,4.62,12.2,"加多被试（脑/腿/手 等）训练后，对没见过的人脚做 OOD 测试：noisy / N2N / NTN / 参考 —— 结构忠实、无脑补")
table(s,0.6,5.2,[3.0,1.7,1.7],[
    ["人脚 OOD 测试(多被试)","N2N","NTN"],
    ["与真值相关 r","0.843","0.839"],
    ["与真值 PSNR","24.67","24.66"],
],fs=11,rh=0.44)
bullets(s,7.6,5.18,5.2,2.1,[
    [("加多被试后的结果：",14,NAVY,True)],
    [("· 没见过的人脚 OOD：N2N≈NTN、结构忠实、",13,INK,False),("无脑补假血管",13,MINT,True),("（r 0.84）;",13,INK,False)],
    [("· 36 个噪声 OOD 场景 NTN−N2N=−0.02dB（持平）;",13,INK,False)],
    [("→ 内容 OOD 靠数据多样性解决；数据足时 N2N 也强，NTN 没拉开。",13,INK,False)],
],sz=13,gap=5)

# ==================== 8 再次调整数据量：limited supervision 优势 ====================
s=slide(); motif_title(s,"再调数据量：数据越少，NTN 优势越大",10)
...
# ==================== 10 后续探索：单图 BSN（5x5x4 成功）====================
s=slide(); motif_title(s,"后续探索：白化后单张图 BSN 可行（5×5×4）",12)
