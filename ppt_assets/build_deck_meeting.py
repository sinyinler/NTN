...
# ==================== 5 一开始怎么用 / 为什么失败 ====================
s=slide(); motif_title(s,"第一版：直接照搬，为什么失败",5)
bullets(s,0.6,1.5,5.4,3.2,[
    [("最初做法：",15,NAVY,True)],
    [("  · D′ 高斯区间设得窄、且只用单一数据集；",13.5,INK,False)],
    [("  · 用 N2N 当伪 GT，指标在 raw 域算；",13.5,INK,False)],
    [("  · 没区分噪声 OOD / 内容 OOD。",13.5,INK,False)],
    [("结果：换噪声 NTN 掉、换部位直接崩。",14.5,RED,True)],
],sz=13.5,gap=7)
table(s,0.6,4.7,[2.7,1.4,1.4],[
    ["人脚(内容OOD)","N2N","NTN"],
    ["与真值相关 r","0.65","−0.15"],
    ["36 OOD 场景 PSNR","24.77","23.88"],
],fs=11.5,rh=0.5)
why(s,0.6,6.45,5.5,"两类 OOD 没拆开、区间太窄、raw 域指标被亮血管放大 → 误判“方法无效”。")
pic(s,os.path.join(R,'metrics_foot5_old','compare_gray_color.png'),6.3,1.6,6.5,3.0)
cap(s,6.3,4.62,6.5,"旧模型测人脚：NTN 与真值反相关 (r=−0.15)，结构崩坏")
pic(s,os.path.join(R,'eval_ood_scenes_191_226_old','vis','192.png'),6.3,5.0,6.5,2.1)
cap(s,6.3,7.08,6.5,"旧模型 3×3 噪声 OOD：NTN 比 N2N 低 0.89dB")

# ==================== 6 修复 + 初步成果 ====================
s=slide(); motif_title(s,"调整后初步跑通：白化生效、不再崩",6)
pic(s,os.path.join(R,'metrics_5x5x1_0_100','compare_gray_color.png'),0.55,1.55,12.2,3.1)
cap(s,0.55,4.66,12.2,"5×5 在分布内：noisy / N2N / NTN / reference —— NTN 与 N2N 视觉、数值持平，噪声被干净去除")
table(s,0.6,5.25,[2.3,1.5,1.5,1.5],[
    ["5×5 在分布内","PSNR","MSSIM","r"],
    ["noisy","22.78","0.430","0.661"],
    ["N2N","30.57","0.838","0.873"],
    ["NTN (ours)","30.39","0.836","0.872"],
],fs=11,rh=0.42)
bullets(s,7.4,5.3,5.4,2.0,[
    [("调整了什么：",14.5,NAVY,True)],
    [("· 实测噪声 → D′ 用够宽盲区间；",13,INK,False)],
    [("· crop/batch 对齐、log1p 域处理与评测；",13,INK,False)],
    [("· 伪干净目标 + 仿射校准评测。",13,INK,False)],
    [("初步成果：NTN≈N2N、白化生效、不再崩。",13.5,MINT,True)],
],sz=13,gap=5)
