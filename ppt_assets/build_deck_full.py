...
def table(s,l,t,colw,rows,fs=10.5,rh=0.44,hl=()):
    tw=sum(colw)
    for i,row in enumerate(rows):
        ...
        for j,cell in enumerate(row):
            col=NAVY if i==0 else (RED if cell.strip().startswith("−") or "崩" in cell else INK)
            text(s,cx+0.12,y+0.085,colw[j]-0.16,0.32,[[(cell,fs,col,i==0)]]); cx+=colw[j]
...
# ============ 18 附录A 单图去噪数值 ============
s=slide(); motif_title(s,"附录 A：单图去噪结果（多被试新模型，PSNR/MSSIM/r）",18)
table(s,0.55,1.55,[2.7,2.5,2.4,2.4,2.4],[
    ("输入图","参考 / 域","noisy","N2N","NTN"),
    ("5x5x4 / f100","reference(raw)","29.79/.729/.817","33.63/.870/.893","33.65/.870/.894"),
    ("5x5x1 / f100","reference(raw)","22.78/.430/.661","30.57/.838/.873","30.39/.836/.872"),
    ("3x3x1 / f100","reference(raw)","−9.64/.099/.035","23.05/.730/.880","23.02/.727/.878"),
    ("305/2_1 脑(训练内)","50帧均值(raw)","20.34/.522/.991","19.60/.725/.996","18.90/.730/.996"),
    ("Basler raw","reference(raw)","30.76/.759/.833","33.26/.873/.902","33.88/.875/.900"),
    ("脚 5x5x5/50","5x5x100(log)","24.10/.475/.939","27.85/.764/.967","27.32/.761/.965"),
],fs=11,hl=(5,))
text(s,0.55,5.0,12.2,1.4,[
    [("· 多数场景 NTN ≈ N2N；",13,INK,False),("Basler raw 上 NTN 略胜(33.88 vs 33.26)",13,MINT,True),("——少数 N2N 偏弱处 NTN 能占优。",13,INK,False)],
    [("· 305(脑)是训练内、且单帧 vs 多帧均值参考 → raw 域 PSNR 反转(尺度敏感)，以 MSSIM/r 为准。",12.5,MUTE,False)],
    [("· 3x3x1 的 noisy PSNR 为负=噪声极强(信噪比极低)，去噪后 r 0.03→0.88。",12.5,MUTE,False)],
],space=1.12)

# ============ 19 附录B 老vs新 + 群体 + 机制 ============
s=slide(); motif_title(s,"附录 B：老 vs 新、群体对照、翻译机制（数值）",19)
text(s,0.55,1.45,12,0.35,[[("① 老/新模型 & 群体（PSNR/MSSIM/r，log1p 域）",13.5,NAVY,True)]])
table(s,0.55,1.85,[3.2,2.7,2.7,2.7,1.5],[
    ("对比","noisy","N2N","NTN","NTN−N2N"),
    ("脚5x5x5 老模型(L100)","24.10/.475/.939","8.98/.635/.653","2.99/.545/−0.15","崩溃"),
    ("脚5x5x5 新模型(L100)","24.10/.475/.939","27.85/.764/.967","27.32/.761/.965","≈"),
    ("3x3x1 36场景 老","21.05/.308/.498","24.77/.807/.908","23.88/.809/.905","−0.89dB"),
    ("3x3x1 36场景 新","21.05/.308/.498","26.81/.816/.917","26.91/.818/.912","+0.10dB"),
],fs=10.5,hl=(2,4))
text(s,0.55,4.55,12,0.35,[[("② 翻译机制诊断（level1，10 场景）",13.5,NAVY,True)]])
bullets(s,0.7,4.95,12.0,1.5,[
    [("去噪 PSNR：",13,NAVY,True),("D'(I)无翻译 14.22  →  D'(T(I)) 20.34  >  N2N 19.31（翻译是“救命”一步，+6dB）。",13,INK,False)],
    [("噪声白化：",13,MINT,True),("空间自相关 0.82→0.13；谱平坦度 0.055→0.51(×9)；std 0.46→0.53——真实散斑被 decorrelate 成近白高斯。",13,INK,False)],
],sz=13,gap=6)

# ============ 20 附录C BSN + 噪声 + 固定σ ============
s=slide(); motif_title(s,"附录 C：BSN、噪声测量、固定-σ 小数据探索",20)
text(s,0.55,1.42,12,0.35,[[("① BSN(ZS-N2N) on 翻译图（log1p 域）",13.5,NAVY,True)]])
table(s,0.55,1.78,[2.5,2.0,2.0,2.4,2.0],[
    ("输入","noisy","BSN(raw)","BSN(translated)","NTN=D'(T)"),
    ("5x5x4(近分布)","17.74/.229","17.82/.233","20.82/.491","21.30/.673"),
    ("3x3x1(超噪OOD)","11.33/.082","11.43/.092","−1.09/.098(T失稳)","9.67/.643"),
],fs=10.5,hl=())
text(s,0.55,3.35,12,0.3,[[("BSN(raw)≈noisy(去不动相关散斑)；T 白化后 BSN 明显可行(5x5x4)；3x3x1 噪声超界、T 失稳。",12,MUTE,False)]])
text(s,0.55,3.85,6.0,0.35,[[("② 实测噪声 σ_mad (log1p)",13.5,NAVY,True)]])
table(s,0.55,4.2,[3.4,2.4],[
    ("数据","σ_mad"),("5x5 L1/L2/L3/L4","0.43/0.21/0.14/0.10"),
    ("脑 / 腿 / 手(325)","0.07 / 0.03 / 0.37"),("3x3x1 / mix-OOD","0.64 / 0.68"),
],fs=10.5,hl=(3,))
text(s,6.6,3.85,6.2,0.35,[[("③ 固定-σ + 小数据探索",13.5,NAVY,True)]])
bullets(s,6.7,4.25,6.0,2.0,[
    [("σ=0.15：",13,RED,True),("NTN 崩（192: ntn 12.5 ≪ n2n 23.0）——T 要把 0.68 硬减到 0.15，做不到。",13,INK,False)],
    [("σ=0.7：",13,NAVY,True),("已重训(D'/T val≈0.037)，OOD 群体评估待跑。",13,INK,False)],
    [("结论：",13,MINT,True),("OOD 噪声(0.64–0.68)远超训练(0.14)；单一固定 σ 不适合宽噪声跨度，盲区间更稳。",13,INK,False)],
],sz=13,gap=6)
